...
- **Keep recent** — when cleaning records, optionally keep the most recent 10 records (off by default for both manual and auto clean). When cleaning logs, the most recent 5 logs are kept by default (auto clean always keeps them; manual clean can be toggled in the tab, on by default).
