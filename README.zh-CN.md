...
可通过"立即清理记录"按钮手动清理（执行前会弹出确认对话框），也可启用自动清理：Jingle 运行期间插件会定期检查（约每分钟一次），文件夹大小一旦超过阈值（默认 50 MB）即自动执行清理，默认关闭。两种方式均可选择保留最近若干条记录而非全部删除，保留数量可自定义，默认为 10。标签页会显示文件夹当前大小与文件数量，并提供打开文件夹的按钮。
...
手动清理默认保留每个实例最近 5 个日志（数量可自定义，取消勾选则全部删除）。自动清理始终保留最近的日志，Jingle 运行期间一旦日志总大小超过独立阈值（同样默认 50 MB，同样默认关闭）即自动执行。插件会自动检测 MultiMC / Prism Launcher 文件夹，若检测结果有误，可通过 Browse 按钮手动指定。每个实例的日志大小与文件数量均显示在标签页中。
